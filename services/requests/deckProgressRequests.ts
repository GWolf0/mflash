...
export async function requestGetDeckProgressByDeckIdAndUserId(deckId: string, userId: string): Promise<DOE<DeckProgressModel | null>> {
...
export async function requestCreateDeckProgress(data: Omit<DeckProgressModel, "id">): Promise<DOE<DeckProgressModel>> {
...
export async function requestUpdateDeckProgress(id: string, data: Partial<Omit<DeckProgressModel, "id">>): Promise<DOE<DeckProgressModel>> {
    const doe: DOE<DeckProgressModel> = { data: null, error: null };
    ...
    try {
        ...
        await db.collection("decksProgress").updateOne(
            { _id: new ObjectId(id) },
            { $set: updateData }
        );
...
export async function requestDeleteDeckProgress(id: string): Promise<DOE<boolean>> {
    ...
    try {
        ...
        const result = await db.collection("decksProgress").deleteOne({ _id: new ObjectId(id) });
...
export async function requestDeleteDeckProgressByUserIdAndDeckId(userId: string, deckId: string): Promise<DOE<number>> {
    const doe: DOE<number> = { data: null, error: null };

    try {
        const db = await getDB();
        if (!db) throw new Error("Database connection failed");

        const result = await db.collection("decksProgress").deleteMany({ user_id: new ObjectId(userId), deck_id: new ObjectId(deckId) });
...
export async function requestDeleteDeckProgressByUserId(userId: string): Promise<DOE<number>> {
    const doe: DOE<number> = { data: null, error: null };
